...
import { TRIGGER_CAMPAIGN_DETAILS_CURRENT_PAGE, TRIGGER_SEARCH_KEYWORD } from "../../redux/actions/actionTypes"
      ...
      <div className="d-flex flex-wrap flex-stack mb-6">
        ...
        {showButtons && (
          <div className="d-flex flex-wrap my-2">
            <div className="me-4">
              <CSVLink data={formatData(campaigns)} className="btn btn-outline-dark btn-sm rounded-pill" filename={`Campaigns${moment.utc().format("DD-MM-YY")}.csv`} target="_blank">
                Export
              </CSVLink>
            </div>
      ...
      <div className="py-3 campaign-table-outer">
        {isLoading ? (
          ...
          <div className="table-responsive">
              ...
              <tbody>
                {currentData.length > 0 && !error ? (
                  currentData.map((item, i) => {
                    return (
                      <tr key={i}>
                        <td className="text-center">{(currentPage - 1) * campaignsPerPage + i + 1}</td>
                        ...
                        <td>
                          <div className="text-truncate text-primary" style={{ width: "100px" }}>
                            <a style={{ width: "100px" }} target="_blank" className="text-primary" href={`https://fedicoms.net/verify-mobile?campaignId=${item._id}`}>{`https://fedicoms.net/verify-mobile?campaignId=${item._id}`}</a>
                          </div>
                        </td>
                        ...
                        <td>
                          <div className="d-flex action-btns justify-content-evenly">
                            <Link to={`/edit-campaign/${item._id}`} state={item} className="btn btn-secondary-outline btn-sm">
                              Edit
                            </Link>
            ...
            {campaigns?.length > campaignsPerPage && searchKey === "" && <PaginationWrappper postsPerPage={campaignsPerPage} totalPosts={campaigns.length} currentPage={currentPage} setCurrentPage={setCurrentPage} data={campaigns} indexOfLastPost={indexOfLastCampaign} indexOfFirstPost={indexOfFirstCampaign} type="campaignTableDashboard" />}
          </div>
        )}
      </div>
    </div>
